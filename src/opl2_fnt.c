...
static struct { uint32_t sjis, ucs; } cctable[] = {
    {0x005C,0x00A5},{0x007E,0x203E},{0x00A1,0xFF61},{0x00A2,0xFF62},
    ...
    {0x00DF,0xFF9F},{0x8140,0x3000},{0x8141,0x3001},{0x8142,0x3002},
    {0x8143,0xFF0C},{0x8144,0xFF0E},{0x8145,0x30FB},{0x8146,0xFF1A},
    {0x8147,0xFF1B},{0x8148,0xFF1F},{0x8149,0xFF01},{0x814A,0x309B},
    {0x814B,0x309C},{0x814C,0x00B4},{0x814D,0xFF40},{0x814E,0x00A8},
    {0x814F,0xFF3E},{0x8150,0xFFE3},{0x8151,0xFF3F},{0x8152,0x30FD},
    {0x8153,0x30FE},{0x8154,0x309D},{0x8155,0x309E},{0x8156,0x3003},
    {0x8157,0x4EDD},{0x8158,0x3005},{0x8159,0x3006},{0x815A,0x3007},
    {0x815B,0x30FC},{0x815C,0x2015},{0x815D,0x2010},{0x815E,0xFF0F},
    {0x815F,0x005C},{0x8160,0x301C},{0x8161,0x2016},{0x8162,0xFF5C},
    {0x8163,0x2026},{0x8164,0x2025},{0x8165,0x2018},{0x8166,0x2019},
    {0x8167,0x201C},{0x8168,0x201D},{0x8169,0xFF08},{0x816A,0xFF09},
    {0x816B,0x3014},{0x816C,0x3015},{0x816D,0xFF3B},{0x816E,0xFF3D},
    {0x816F,0xFF5B},{0x8170,0xFF5D},{0x8171,0x3008},{0x8172,0x3009},
    {0x8173,0x300A},{0x8174,0x300B},{0x8175,0x300C},{0x8176,0x300D},
    {0x8177,0x300E},{0x8178,0x300F},{0x8179,0x3010},{0x817A,0x3011},
    {0x817B,0xFF0B},{0x817C,0x2212},{0x817D,0x00B1},{0x817E,0x00D7},
    ...
    {0x8190,0xFF04},{0x8191,0x00A2},{0x8192,0x00A3},{0x8193,0xFF05},
    ...
    {0x81AC,0x3013},{0x81B8,0x2208},{0x81B9,0x220B},{0x81BA,0x2286},
    {0x81BB,0x2287},{0x81BC,0x2282},{0x81BD,0x2283},{0x81BE,0x222A},
    {0x81BF,0x2229},{0x81C8,0x2227},{0x81C9,0x2228},{0x81CA,0x00AC},
    {0x81CB,0x21D2},{0x81CC,0x21D4},{0x81CD,0x2200},{0x81CE,0x2203},
    {0x81DA,0x2220},{0x81DB,0x22A5},{0x81DC,0x2312},{0x81DD,0x2202},
    {0x81DE,0x2207},{0x81DF,0x2261},{0x81E0,0x2252},{0x81E1,0x226A},
    {0x81E2,0x226B},{0x81E3,0x221A},{0x81E4,0x223D},{0x81E5,0x221D},
    {0x81E6,0x2235},{0x81E7,0x222B},{0x81E8,0x222C},{0x81F0,0x212B},
    {0x81F1,0x2030},{0x81F2,0x266F},{0x81F3,0x266D},{0x81F4,0x266A},
    {0x81F5,0x2020},{0x81F6,0x2021},{0x81F7,0x00B6},{0x81FC,0x25EF},
    {0x824F,0xFF10},{0x8250,0xFF11},{0x8251,0xFF12},{0x8252,0xFF13},
    {0x8253,0xFF14},{0x8254,0xFF15},{0x8255,0xFF16},{0x8256,0xFF17},
    {0x8257,0xFF18},{0x8258,0xFF19},{0x8260,0xFF21},{0x8261,0xFF22},
    {0x8262,0xFF23},{0x8263,0xFF24},{0x8264,0xFF25},{0x8265,0xFF26},
    {0x8266,0xFF27},{0x8267,0xFF28},{0x8268,0xFF29},{0x8269,0xFF2A},
    {0x826A,0xFF2B},{0x826B,0xFF2C},{0x826C,0xFF2D},{0x826D,0xFF2E},
    {0x826E,0xFF2F},{0x826F,0xFF30},{0x8270,0xFF31},{0x8271,0xFF32},
    {0x8272,0xFF33},{0x8273,0xFF34},{0x8274,0xFF35},{0x8275,0xFF36},
    {0x8276,0xFF37},{0x8277,0xFF38},{0x8278,0xFF39},{0x8279,0xFF3A},
    ...
    {0x8299,0xFF59},{0x829A,0xFF5A},{0x829F,0x3041},{0x82A0,0x3042},
    {0x82A1,0x3043},{0x82A2,0x3044},{0x82A3,0x3045},{0x82A4,0x3046},
    {0x82A5,0x3047},{0x82A6,0x3048},{0x82A7,0x3049},{0x82A8,0x304A},
    {0x82A9,0x304B},{0x82AA,0x304C},{0x82AB,0x304D},{0x82AC,0x304E},
    {0x82AD,0x304F},{0x82AE,0x3050},{0x82AF,0x3051},{0x82B0,0x3052},
    {0x82B1,0x3053},{0x82B2,0x3054},{0x82B3,0x3055},{0x82B4,0x3056},
    {0x82B5,0x3057},{0x82B6,0x3058},{0x82B7,0x3059},{0x82B8,0x305A},
    {0x82B9,0x305B},{0x82BA,0x305C},{0x82BB,0x305D},{0x82BC,0x305E},
    {0x82BD,0x305F},{0x82BE,0x3060},{0x82BF,0x3061},{0x82C0,0x3062},
    {0x82C1,0x3063},{0x82C2,0x3064},{0x82C3,0x3065},{0x82C4,0x3066},
    {0x82C5,0x3067},{0x82C6,0x3068},{0x82C7,0x3069},{0x82C8,0x306A},
    {0x82C9,0x306B},{0x82CA,0x306C},{0x82CB,0x306D},{0x82CC,0x306E},
    {0x82CD,0x306F},{0x82CE,0x3070},{0x82CF,0x3071},{0x82D0,0x3072},
    {0x82D1,0x3073},{0x82D2,0x3074},{0x82D3,0x3075},{0x82D4,0x3076},
    {0x82D5,0x3077},{0x82D6,0x3078},{0x82D7,0x3079},{0x82D8,0x307A},
    {0x82D9,0x307B},{0x82DA,0x307C},{0x82DB,0x307D},{0x82DC,0x307E},
    {0x82DD,0x307F},{0x82DE,0x3080},{0x82DF,0x3081},{0x82E0,0x3082},
    {0x82E1,0x3083},{0x82E2,0x3084},{0x82E3,0x3085},{0x82E4,0x3086},
    {0x82E5,0x3087},{0x82E6,0x3088},{0x82E7,0x3089},{0x82E8,0x308A},
    {0x82E9,0x308B},{0x82EA,0x308C},{0x82EB,0x308D},{0x82EC,0x308E},
    {0x82ED,0x308F},{0x82EE,0x3090},{0x82EF,0x3091},{0x82F0,0x3092},
    {0x82F1,0x3093},{0x8340,0x30A1},{0x8341,0x30A2},{0x8342,0x30A3},
    {0x8343,0x30A4},{0x8344,0x30A5},{0x8345,0x30A6},{0x8346,0x30A7},
    {0x8347,0x30A8},{0x8348,0x30A9},{0x8349,0x30AA},{0x834A,0x30AB},
    {0x834B,0x30AC},{0x834C,0x30AD},{0x834D,0x30AE},{0x834E,0x30AF},
    {0x834F,0x30B0},{0x8350,0x30B1},{0x8351,0x30B2},{0x8352,0x30B3},
    {0x8353,0x30B4},{0x8354,0x30B5},{0x8355,0x30B6},{0x8356,0x30B7},
    {0x8357,0x30B8},{0x8358,0x30B9},{0x8359,0x30BA},{0x835A,0x30BB},
    {0x835B,0x30BC},{0x835C,0x30BD},{0x835D,0x30BE},{0x835E,0x30BF},
    {0x835F,0x30C0},{0x8360,0x30C1},{0x8361,0x30C2},{0x8362,0x30C3},
    {0x8363,0x30C4},{0x8364,0x30C5},{0x8365,0x30C6},{0x8366,0x30C7},
    {0x8367,0x30C8},{0x8368,0x30C9},{0x8369,0x30CA},{0x836A,0x30CB},
    {0x836B,0x30CC},{0x836C,0x30CD},{0x836D,0x30CE},{0x836E,0x30CF},
    {0x836F,0x30D0},{0x8370,0x30D1},{0x8371,0x30D2},{0x8372,0x30D3},
    {0x8373,0x30D4},{0x8374,0x30D5},{0x8375,0x30D6},{0x8376,0x30D7},
    {0x8377,0x30D8},{0x8378,0x30D9},{0x8379,0x30DA},{0x837A,0x30DB},
    {0x837B,0x30DC},{0x837C,0x30DD},{0x837D,0x30DE},{0x837E,0x30DF},
    ...
    {0x8394,0x30F4},{0x8395,0x30F5},{0x8396,0x30F6},{0x839F,0x0391},
    ...
    {0x83B4,0x03A7},{0x83B5,0x03A8},{0x83B6,0x03A9},{0x83BF,0x03B1},
    ...
    {0x83D4,0x03C7},{0x83D5,0x03C8},{0x83D6,0x03C9},{0x8440,0x0410},
    {0x8441,0x0411},{0x8442,0x0412},{0x8443,0x0413},{0x8444,0x0414},
    {0x8445,0x0415},{0x8446,0x0401},{0x8447,0x0416},{0x8448,0x0417},
    {0x8449,0x0418},{0x844A,0x0419},{0x844B,0x041A},{0x844C,0x041B},
    {0x844D,0x041C},{0x844E,0x041D},{0x844F,0x041E},{0x8450,0x041F},
    {0x8451,0x0420},{0x8452,0x0421},{0x8453,0x0422},{0x8454,0x0423},
    {0x8455,0x0424},{0x8456,0x0425},{0x8457,0x0426},{0x8458,0x0427},
    {0x8459,0x0428},{0x845A,0x0429},{0x845B,0x042A},{0x845C,0x042B},
    {0x845D,0x042C},{0x845E,0x042D},{0x845F,0x042E},{0x8460,0x042F},
    ...
    {0x847C,0x043B},{0x847D,0x043C},{0x847E,0x043D},{0x8480,0x043E},
    {0x8481,0x043F},{0x8482,0x0440},{0x8483,0x0441},{0x8484,0x0442},
    {0x8485,0x0443},{0x8486,0x0444},{0x8487,0x0445},{0x8488,0x0446},
    {0x8489,0x0447},{0x848A,0x0448},{0x848B,0x0449},{0x848C,0x044A},
    {0x848D,0x044B},{0x848E,0x044C},{0x848F,0x044D},{0x8490,0x044E},
    {0x8491,0x044F},{0x849F,0x2500},{0x84A0,0x2502},{0x84A1,0x250C},
    {0x84A2,0x2510},{0x84A3,0x2518},{0x84A4,0x2514},{0x84A5,0x251C},
    {0x84A6,0x252C},{0x84A7,0x2524},{0x84A8,0x2534},{0x84A9,0x253C},
    {0x84AA,0x2501},{0x84AB,0x2503},{0x84AC,0x250F},{0x84AD,0x2513},
    {0x84AE,0x251B},{0x84AF,0x2517},{0x84B0,0x2523},{0x84B1,0x2533},
    {0x84B2,0x252B},{0x84B3,0x253B},{0x84B4,0x254B},{0x84B5,0x2520},
    {0x84B6,0x252F},{0x84B7,0x2528},{0x84B8,0x2537},{0x84B9,0x253F},
    {0x84BA,0x251D},{0x84BB,0x2530},{0x84BC,0x2525},{0x84BD,0x2538},
    {0x84BE,0x2542},{0x889F,0x4E9C},{0x88A0,0x5516},{0x88A1,0x5A03},
...
static inline uint32_t sjis2ucs(uint16_t c)
{
    ...
    //DEBUGPRINT("%s: mapping S-JIS character 0x%04x... ", __func__, c);

    if(cctable[a].sjis == c)
        return cctable[a].ucs;
    if(cctable[b].sjis == c)
        return cctable[b].ucs;
    ...
    while((b - a) > 1)
    {
        ...
        if(cctable[m].sjis == c)
        {
            //DEBUGPRINT("mapped to 0x%04x\n", cctable[m].ucs);

            return cctable[m].ucs;
        }
...
static pl2Font *pl2FontLoadInternal(const uint8_t *data)
{
    ...
    pl2Font *font = NEW(pl2Font);

    font->magic = READUINT32(data);

    if(font->magic != PL2_FNT_MAGIC)
    {
        pl2FontFree(font);
        PL2_ERROR(PL2_ERR_FORMAT);
    }

    font->numGlyphs = READUINT32(data);
    font->glyphSize = READUINT32(data);
    READUINT32(data);
    ...
    int size = font->glyphSize * font->glyphSize;

    font->glyphs = NEWARR(font->numGlyphs * size, uint8_t);
...
pl2Font *pl2FontLoad(const char *name)
{
    ...
    snprintf(temp, sizeof(temp), "%s.bin", name);
    ...
    pl2Font *font = pl2FontLoadInternal(file->data);
...
pl2Font *pl2FontLoadFile(const char *name)
{
    ...
    pl2Font *font = pl2FontLoadInternal(data);
